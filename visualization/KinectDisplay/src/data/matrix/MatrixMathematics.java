...
/**
 * 
 * The class taken from CodeProject article.
 * url: http://www.codeproject.com/Articles/405128/Matrix-operations-in-Java
 * 
 * Published under CPOL 1.02 <http://www.codeproject.com/info/cpol10.aspx>
 * 
 * @author Ata Amini 
 *
 *
 * --------------------------------------------------------------------------
 * 17.07.2013. Mirko Raca
 * 
 * Added code for multiplying a quaternion from JOGAMP package.
 * --------------------------------------------------------------------------
 */
...
public class MatrixMathematics {
	...
	public static Quaternion multiply(Matrix matrix1, Quaternion q, boolean normalize) throws IllegalDimensionException{
		if( matrix1.getNcols() != 4 || matrix1.getNrows() != 4 )
			throw new IllegalDimensionException("Transformation matrix has to be 4x4");
		// convert to 4x1 matrix
		Matrix quatMat = new Matrix(4, 1);
		Quaternion q1 = QuaternionMathematics.normByW(q);
		quatMat.setValueAt(0, 0, q1.getX());
		quatMat.setValueAt(1, 0, q1.getY());
		quatMat.setValueAt(2, 0, q1.getZ());
		quatMat.setValueAt(3, 0, q1.getW());
		Matrix res = multiply(matrix1, quatMat);
		Quaternion qOut = new Quaternion();
		qOut.setX( (float)res.getValueAt(0, 0) );
		qOut.setY( (float)res.getValueAt(1, 0) );
		qOut.setZ( (float)res.getValueAt(2, 0) );
		qOut.setW( (float)res.getValueAt(3, 0) );
		if(normalize)
			qOut = QuaternionMathematics.normByW(qOut);
		return qOut;
	}
}
